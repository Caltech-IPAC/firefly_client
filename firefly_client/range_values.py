...
class RangeValues:
    ...
    STRETCH_ALGORITHM_DICT: ClassVar = {
        "linear": 44,
        "log": 45,
        "loglog": 46,
        "equal": 47,
    ...
    INVERSE_STRETCH_ALGORITHM: ClassVar = {
        v: k for k, v in STRETCH_ALGORITHM_DICT.items()
    }
    ...
    def create_rv(
        ...
        if asinh_stretch is None or math.isnan(asinh_stretch):
            ...
            raise ValueError(
                "invalid asinh_stretch for hue-preserving rgb: %f" % asinh_stretch
            )
        else:
    ...
    def create_rv_by_stretch_type(cls, algorithm, stretch_type, **additional_params):
        ...
        if st == "zscale":
            ...
            return cls.create_rv_standard(
                a, "percent", lower_value=0, upper_value=100, **additional_params
            )
        else:
            return cls.create_rv_standard(a, stretch_type, **additional_params)

    @classmethod
    def create_rv_standard(
        cls,
        algorithm,
        ...
        **additional_params : optional keyword arguments
        ...
        """
        retval = cls.create_rv(
            stretch_type, lower_value, upper_value, algorithm, **additional_params
        )
        if not retval:
            t = (
                stretch_type
                if stretch_type.lower() in cls.STRETCH_TYPE_DICT
                else "percent"
            )
            a = (
                algorithm
                if algorithm.lower() in cls.STRETCH_ALGORITHM_DICT
                else "linear"
            )
    ...
    def parse_rvstring(cls, rvstring):
        ...
        outdict = dict(
            lower_type=cls.INVERSE_STRETCH_TYPE[int(vals[0])],
