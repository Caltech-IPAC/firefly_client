...
def debug(s):
    DebugMarker.firefly_client_debug and print("DEBUG: %s" % s)
...
def dict_to_str(in_dict):
    return json.dumps(in_dict, indent=2, default=str)
...
def create_image_url(image_source):
    ...
    if not image_source.startswith("data:image") and not is_url(image_source):
        ...
        with open(image_source, "rb") as fp:
            data = fp.read()
            data_uri = b"".join(base64.urlsafe_b64encode(data).splitlines())
...
def ensure3(val, name):
    ...
    ret = val if isinstance(val, list) else [val, val, val]
    if not len(ret) == 3:
