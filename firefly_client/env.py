...
SUGGESTION = (
    "fix jupyter_firefly_extensions in Jupyter Lab or use FireflyClient.make_client()"
)
...
class Env:
    ...
    def raise_invalid_lab_error(cls):
        if not cls.firefly_lab_extension:
            raise RuntimeError(
                "FireflyClient.makeLabClient can only be used in the Jupyterlab environment. "
                + SUGGESTION
            )
    ...
    def show_start_browser_tab_msg(cls, url):
        ...
        print(
            "To start a new tab you you will have to disable popup blocking for this site."
        )
    ...
    def resolve_client_channel(cls, in_channel):
        if in_channel is not None:
            ...
            return (
                base64.urlsafe_b64encode(start_str.encode()).decode().replace("=", "")
            )
    ...
    def failed_net_message(cls, location, status_code=-1):
        ...
        if cls.firefly_lab_extension and cls.firefly_url_lab:
            ...
            err_message += (
                "Check setting of FIREFLY_URL environment variable: %s"
                % cls.firefly_url
            )
        return err_message
